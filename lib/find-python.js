...
class Logger {
  constructor () {
    /** @private */
    this._npmlog = npmlog
    this.log = logWithPrefix(this._npmlog, 'find Python')
  ...
  /**
   * Logs a message at verbose level, but also saves it to internal buffer
   * from which they can be accessed later by calling `this.dumpErrorLog()`.
  ...
  */
  ...
  /**
   * Clear buffer with error messages and return colleted messages about error as a string
   * To add messages to error buffer use `this.addLog()`
   *
   * @returns string contain all collected error messages
   */
  dumpErrorLog () {
    const errString = this._errorLog.map((str) => str.trim()).join('\n')
    this._errorLog = []
    return errString
  }
 ...
 */
  colorizeOutput (color, string) {
    return color + string + Logger.colors.RESET
  }
}

// `standard` package version we using doesn't work
// with class property declaration syntax.
// As soon as we upgrade standard, this should be
// declared as static property of `Logger` class
Logger.colors = {
  RED: '\x1b[31m',
  GREEN: '\x1b[32m',
  RESET: '\x1b[0m'
...
class PythonFinder {
    ...
    const checks = [
        ...
        before: (name) => {
          ...
          this.logger.addLog(
            '- "--python=" or "npm config get python" is ' +
              `"${this.logger.colorizeOutput(Logger.colors.GREEN, this.configPython)}"`
          )
        },
        checkFunc: this.checkCommand,
      ...
      },
      {
        before: (name) => {
          if (!this.env.PYTHON) {
            this.logger.addLog(
              `Python is not set from environment variable ${this.logger.colorizeOutput(
                Logger.colors.GREEN,
                'PYTHON'
              )}`
            )
          ...
          this.logger.addLog(
            `${this.logger.colorizeOutput(
              Logger.colors.GREEN,
              'process.env.PYTHON'
            )} is "${this.logger.colorizeOutput(Logger.colors.GREEN, this.env.PYTHON)}"`
          )
        },
        checkFunc: this.checkCommand,
        arg: this.env.PYTHON,
    ...
    if (this.win) {
      for (let i = 0; i < this.winDefaultLocations.length; ++i) {
        ...
        checks.push({
          before: () => {
            this.logger.addLog(
              `checking if Python is "${this.logger.colorizeOutput(Logger.colors.GREEN, location)}"`
            )
          },
      ...
      checks.push({
        before: () => {
          this.logger.addLog(
            `checking if the ${this.logger.colorizeOutput(
              Logger.colors.GREEN,
              'py launcher'
  ...
  async runChecks (checks) {
    ...
    for (const check of checks) {
      ...
      try {
        if (!check.before) {
          this.logger.addLog(
            `checking if "${this.logger.colorizeOutput(
              Logger.colors.GREEN,
        ...
        this.logger.log.verbose(
          `executing "${this.logger.colorizeOutput(
            Logger.colors.GREEN,
  ...
  checkExecPath (execPath) {
    ...
    return new Promise((resolve, reject) => {
      this.logger.log.verbose(`executing "${this.logger.colorizeOutput(Logger.colors.GREEN, execPath)}" to get version`)

      // do not wrap with quotes because executing without shell
      this.run(execPath, this.argsVersion, false)
        .then((ver) => {
          this.logger.log.silly(this.logger.colorizeOutput(Logger.colors.GREEN, 'got version:'), ver)
          ...
          try {
            ...
            this.logger.log.silly(`semver.satisfies(${ver}, ${this.semverRange}) threw:\n${err.stack}`)
            this.logger.addLog(`"${this.logger.colorizeOutput(Logger.colors.RED, execPath)}" does not have a valid version`)
            this.logger.addLog('Is it a Python executable?')
          ...
          if (!valid) {
            reject(new PythonVersionError({ received: ver, wanted: this.semverRange }, `Found unsupported Python version ${ver}`))
          }
    ...
    return new Promise(
      ...
      function (resolve, reject) {
        ...
        function execFileCallback (err, stdout, stderr) {
          ...
          this.logger.log.silly(`${this.logger.colorizeOutput(Logger.colors.RED, 'execFile result')}: err =`, (err && err.stack) || err)
          this.logger.log.verbose(`${this.logger.colorizeOutput(Logger.colors.RED, 'execFile result')}: stdout =`, stdout)
          this.logger.log.silly(`${this.logger.colorizeOutput(Logger.colors.RED, 'execFile result')}: stderr =`, stderr)
  ...
  catchErrors (err, check) {
    this.logger.addLog(this.logger.colorizeOutput(Logger.colors.RED, `FAIL: ${check.name || check.arg}`))
    ...
    // @ts-ignore
    if (catchedErrorsCods.includes(err.error && err.error.code)) {
      const { error } = err
      // @ts-ignore
        ...
        case 'ENOENT':
          this.logger.addLog(
            `${this.logger.colorizeOutput(
              ...
              'ERROR:'
              // @ts-ignore
            )} No such file or directory: "${this.logger.colorizeOutput(Logger.colors.RED, error.path)}"`
          )
          break
      ...
      this.logger.addLog(
        `${this.logger.colorizeOutput(Logger.colors.RED, 'ERROR:')} ${
          err ? (err.message || err) : ''
        }`
      )
      this.logger.log.silly(this.logger.colorizeOutput(Logger.colors.RED, 'FULL ERROR:'), err ? (err.stack || err) : '')
  ...
  succeed (execPath, version) {
    this.logger.log.info(
      `using Python version ${this.logger.colorizeOutput(
        Logger.colors.GREEN,
        version
      )} found at "${this.logger.colorizeOutput(Logger.colors.GREEN, execPath)}"`
    )
   ...
   */
  fail () {
    const errorLog = this.logger.dumpErrorLog()
 ...
 */
  log (logger) {
    ...
    for (const prop in this.data) {
      if (this.data[prop]) {
        logger.addLog(`${logger.colorizeOutput(Logger.colors.RED, `${prop.toUpperCase()}:`)} ${this.data[prop].trim()}`)
      }
    }
  }
}

class PythonVersionError extends ErrorWithData {
  /**
   *
   * @param {{wanted: string, received: string}} ver
   * @param {string} message
   */
  constructor (ver, message) {
    super({ messageOrError: message, data: { version: ver.received } })
  ...
  log (logger) {
    if (!this.data.version) {
      logger.addLog(this.message)
    } else {
      logger.addLog(
        `version is ${logger.colorizeOutput(
          Logger.colors.RED,
          this.data.version
        )} - should be ${logger.colorizeOutput(Logger.colors.GREEN, this._ver.wanted)}`
      )

      logger.addLog('')
      logger.addLog(logger.colorizeOutput(Logger.colors.RED, 'THIS VERSION OF PYTHON IS NOT SUPPORTED'))
      logger.addLog('')
    }
  }
}
...
/* findPython(null, (err, found) => {
  console.log('found:', (new Logger()).colorizeOutput(Logger.colors.GREEN, found))
  console.log('err:', err)
}) */
